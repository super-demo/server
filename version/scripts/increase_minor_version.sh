...
# Increase by one
new_value=$((minor_value + 1))

# Replace the old constants.go to a new one with new value.
sed "s/const MinorVersion = $minor_value/const MinorVersion = $new_value/g" "$constant_file" > tempfile && mv tempfile "$constant_file"
echo "Minor version has been increased to $new_value"
